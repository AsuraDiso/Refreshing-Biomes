...
local bunches = require "map/mod_bunches"
...
local function checkIfValidWORLD_TILE(world, x, z, valid_tile_types, water)
	...
	if x > OCEAN_POPULATION_EDGE_DIST and x < (WIDTH - OCEAN_POPULATION_EDGE_DIST) and z > OCEAN_POPULATION_EDGE_DIST and z < (HEIGHT - OCEAN_POPULATION_EDGE_DIST) then
		...
		if original_tile_type ~= WORLD_TILES.INVALID and original_tile_type ~= WORLD_TILES.IMPASSABLE then
...
local function placeitemoffgrids(world, x1,z1, data)
    ...
    if x and z and checkIfValidWORLD_TILE(world, x, z, data.valid_tile_types, data.water) and checkforblockingitems(x,z) then
...
function ModBunchSpawnerInit(ents, map_width, map_height)
    entities = ents
    WIDTH = map_width
    HEIGHT = map_height
end

function ModBunchSpawnerRunSingleBatchSpawner(world, spawner_prefab, x, z, add_entity_fn)
    bunch = {}
...
function ModBunchSpawnerRun(world, add_entity_fn)
    for spawner_prefab, _ in pairs(bunches.Bunches)do
        if entities[spawner_prefab] then
            for i, ent in ipairs(entities[spawner_prefab]) do
                ModBunchSpawnerRunSingleBatchSpawner(world, spawner_prefab, ent.x, ent.z, add_entity_fn)
            end
        end
    end
...
function IsModBunchSpawner(prefab)
