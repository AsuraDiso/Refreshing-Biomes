...
local function GetFaceTargetFn(inst)
    if not BrainCommon.ShouldSeekSalt(inst) then
        ...
        return target ~= nil and not (target:HasTag("notarget") or target:HasTag("swampdef")) and target or nil
    end
end

local function KeepFaceTargetFn(inst, target)
    return not BrainCommon.ShouldSeekSalt(inst)
        and not (target:HasTag("notarget") or target:HasTag("swampdef"))
        and inst:IsNear(target, KEEP_FACE_DIST)
end

local function ShouldRunAway(guy)
    return guy:HasTag("character") and not (guy:HasTag("notarget") or guy:HasTag("swampdef"))
end

local FumeagatorBrain = Class(Brain, function(self, inst)
    Brain._ctor(self, inst)
end)

function FumeagatorBrain:OnStart()
