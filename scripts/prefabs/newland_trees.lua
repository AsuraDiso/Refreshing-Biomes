...
function tree_common.CreateTreePrefabs(def)
    ...
    local short_anims = def.anims and def.anims.short or makeanims("short")
    local tall_anims = def.anims and def.anims.tall or makeanims("tall")
    local normal_anims = def.anims and def.anims.normal or makeanims("normal")
    local monster_anims = def.anims and def.anims.monster or makeanims("monster")
    ...
    local stage_fns = {
        short = { fn = SetShort, growfn = GrowShort, time = 1 },
        normal = { fn = SetNormal, growfn = GrowNormal, time = 2 },
    ...
    if not growth_stages then
        ...
        for _, st in ipairs(stages_to_use) do
            local sf = stage_fns[st]
            if sf then
                table.insert(growth_stages, {
                    name = st,
                    time = function(inst) return GetRandomWithVariance(TUNING.DECIDUOUS_GROW_TIME[sf.time].base, TUNING.DECIDUOUS_GROW_TIME[sf.time].random) end,
                    fn = sf.fn,
                    growfn = sf.growfn
                })
            end
    ...
    local function chop_down_tree(inst, chopper)
        ...
        if def.make_monster ~= false and not inst.monster and inst.leaf_state ~= "barren" and inst.components.growable ~= nil and inst.components.growable.stage == 3 and days_survived >= TUNING.DECID_MONSTER_MIN_DAY then
            local chance =
    ...
    local function onburntchanges(inst)
        ...
        inst.MiniMapEntity:SetIcon(def.minimap_icon_burnt or "tree_leaf_burnt.png")
    ...
    local function StartMonster(inst, force, starttimeoffset)
        if def.make_monster == false then return end
    ...
    local function OnTimerDone(inst, data)
        if data.name == "decay" then
            ...
            if inst:IsAsleep() then
                local leftone = false
                for _, v in ipairs(TheSim:FindEntities(x, y, z, 6, DECAYREMOVE_MUST_TAGS, DECAYREMOVE_CANT_TAGS)) do
    ...
    local function onload(inst, data)
        if data ~= nil then
            ...
            local stage_name = inst.components.growable and growth_stages[inst.components.growable.stage] and growth_stages[inst.components.growable.stage].name or "tall"
            inst.anims =
    ...
    local function OnHaunt(inst, haunter)
        ...
        elseif isstump and
            inst.components.workable ~= nil and
            math.random() <= TUNING.HAUNT_CHANCE_SEVERAL then
            inst.components.workable:WorkedBy(haunter, 1)
            inst.components.hauntable.hauntvalue = TUNING.HAUNT_SMALL
            return true
        elseif def.make_monster ~= false and not (isstump or inst.monster) and
    ...
    local function makefn(build, stage, data)
        return function()
            ...
            inst.MiniMapEntity:SetIcon(def.minimap_icon or "tree_leaf.png")
            ...
            inst:AddTag("cattoyairborne")
            inst:AddTag(def.tree_tag)
            inst:AddTag("shelter")

            inst.build = build
            inst.AnimState:SetBank(def.anim_bank)
            inst.AnimState:SetBuild(def.anim_build)
            if def.hue ~= nil then
                inst.AnimState:SetHue(def.hue)
            end
            ...
            inst:SetPrefabName(def.prefab_name)
            ...
            inst:SetStateGraph(def.stategraph or "SGdeciduoustree")
            inst.sg:GoToState("empty")
            ...
            inst.components.growable:SetStage(stage == 0 and math.random(1, #growth_stages) or stage)
    ...
    local function tree(name, build, stage, data)
        return Prefab(name, makefn(build, stage, data), assets)
    end

    local prefabs_to_return = {
        tree(def.prefab_name, "normal", 0),
        tree(def.prefab_name .. "_burnt", "normal", 0, "burnt"),
        tree(def.prefab_name .. "_stump", "normal", 0, "stump")
    }

    for i, v in ipairs(growth_stages) do
        table.insert(prefabs_to_return, tree(def.prefab_name .. "_" .. v.name, "normal", i))
    end

    return unpack(prefabs_to_return)
end

local tree_defs = require("prefabs/newland_tree_defs")

return tree_common.CreateTreePrefabs(tree_defs.swamptrees),
       tree_common.CreateTreePrefabs(tree_defs.savannatrees),
       tree_common.CreateTreePrefabs(tree_defs.cactustrees)
