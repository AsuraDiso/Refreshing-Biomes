...
local function _TerraformTile(inst, tx, ty)
    ...
    if not IsOceanTile(current_tile) and current_tile ~= WORLD_TILES.SWAMP then
        local undertile = _world.components.undertile
        local current_undertile = undertile:GetTileUnderneath(tx, ty)

        _map:SetTile(tx, ty, WORLD_TILES.SWAMP)
...
local function _RevertTile(inst, tx, ty)
    ...
    -- local explosion_cover = SpawnPrefab("swamp_terraformer_explosion")
...
local function terraformer_addtask(inst, tx, ty, time, facing, is_revert)
    ...
    local tile_is_swamp = (tile == WORLD_TILES.SWAMP)
    if (is_revert and tile_is_swamp)
            or (not is_revert and not IsOceanTile(tile) and not tile_is_swamp) then

        local visuals = nil
        if not is_revert then
            visuals = SpawnPrefab("swamp_terraformer_visual")
...
local function terraformer_visuals_finish(inst)
    ...
    local explosion = SpawnPrefab("spider_heal_fx")
...
local function SpawnHealFx(inst)
    ...
    local offset_x = (math.random() * 2 - 1) * 2
    local offset_z = (math.random() * 2 - 1) * 2

    local fx = SpawnPrefab("greattreehealfx")
    if fx ~= nil then
        fx.Transform:SetPosition(x + offset_x, y, z + offset_z)
    end  
    inst:DoTaskInTime(math.random() * 1.5, SpawnHealFx)
end
...
local function terraformvisualsfn()
    ...
    anim_state:SetMultColour(0.5, 0.5, 0, 1)
    anim_state:SetHue(.7)
    inst:AddTag("birdblocker")
    inst:AddTag("FX")
    inst:AddTag("ignorewalkableplatforms")
...
return Prefab("swamp_terraformer", terraformerfn),
    Prefab("swamp_terraformer_visual", terraformvisualsfn)